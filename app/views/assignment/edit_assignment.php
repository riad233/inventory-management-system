...
<div class="list-page-header">
    <h2><i class="fas fa-exchange-alt"></i> Edit Assignment</h2>
  ...
  <div class="col-md-7 col-lg-5">
    <div class="form-page-card">
      ...
      <div class="card-body p-4">
        <form method="post" action="?url=assignment/edit/<?php echo e($assignment['Assignment_ID'] ?? ''); ?>">
          <?php echo csrf_field(); ?>
          <div class="mb-3">
            ...
            <select name="asset_id" class="form-select form-select-sm" required>
              ...
              <?php if(isset($data['assets']) && is_array($data['assets'])): ?>
                <?php foreach($data['assets'] as $asset): ?>
                  <option value="<?php echo e($asset['Asset_ID']); ?>" <?php echo ($assignment['Asset_ID'] ?? '') == $asset['Asset_ID'] ? 'selected' : ''; ?>>
            ...
            <select name="user_id" class="form-select form-select-sm" required>
              ...
              <?php if(isset($data['employees']) && is_array($data['employees'])): ?>
                <?php foreach($data['employees'] as $emp): ?>
                  <option value="<?php echo e($emp['User_ID']); ?>" <?php echo ($assignment['User_ID'] ?? '') == $emp['User_ID'] ? 'selected' : ''; ?>>
                    <?php echo e($emp['Name']); ?>
                  </option>
                <?php endforeach; ?>
              <?php endif; ?>
          ...
          <div class="mb-3">
            ...
            <select name="dept_id" class="form-select form-select-sm" required>
              ...
              <?php echo DropdownHelper::renderOptions('departments', $assignment['Department_ID'] ?? null); ?>
            </select>
          </div>
          <div class="mb-3">
            <label class="form-label">Expected Return Date</label>
            <input type="date" name="exp_return_date" class="form-control form-control-sm"
              value="<?php echo e($assignment['Expected_Return_Date'] ?? ''); ?>" required>
          </div>
