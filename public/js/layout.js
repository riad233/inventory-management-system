...
// Alert Bell Icon Functionality - OUTSIDE DOMContentLoaded
document.addEventListener('DOMContentLoaded', function() {
    ...
    const alertDropdownContent = document.getElementById('alertDropdownContent');
    ...
    window.loadAlerts = function() {
        ...
        fetch('?url=alert/getAlerts')
            ...
            .then(data => {
                ...
                if (data.success) {
                    window.updateAlertBadge(data.count);
                    window.populateAlertDropdown(data.alerts);
                } else {
                    console.error('API returned error:', data.error);
                    if (alertDropdownContent) {
                        alertDropdownContent.innerHTML = '<div class="alert-empty">Error: ' + (data.error || 'Unknown error') + '</div>';
                    }
                }
            })
            .catch(error => {
                ...
                if (alertDropdownContent) {
                    alertDropdownContent.innerHTML = '<div class="alert-empty">Error loading alerts</div>';
                }
            });
    };

    window.updateAlertBadge = function(count) {
        if (alertBadge) {
    ...
    window.populateAlertDropdown = function(alerts) {
        if (!alertDropdownContent) return;

        if (alerts.length === 0) {
            alertDropdownContent.innerHTML = '<div class="alert-empty">No alerts available</div>';
            return;
        }

        let html = '';
        alerts.forEach(alert => {
            const alertClass = alert.alert_type === 'Out of Stock' ? 'out-of-stock' : 'low-stock';
            html += `
                <div class="alert-item">
                    <div class="alert-item-content">
                        <div class="alert-item-name">${window.escapeHtml(alert.name)}</div>
                        <div class="alert-item-info">Stock: <strong>${alert.quantity}</strong></div>
                        <span class="alert-item-message ${alertClass}">${alert.alert_type}</span>
                    </div>
                </div>
            `;
        });

        alertDropdownContent.innerHTML = html;
    };
    ...
    if (alertBellBtn && alertDropdown) {
        ...
        alertBellBtn.addEventListener('click', function(e) {
            e.stopPropagation();
        ...
        alertDropdown.addEventListener('click', function(e) {
            e.stopPropagation();
        });
